...
chart_options = {
    ...
    "차트 10 — PIAAC 다중회귀분석 (보완 분석)": "chart_regression",  # ← 신규 추가
}
...
def render_chart9():
    ...
    ax.set_ylim(0, 0.5); ax.legend(fontsize=9); ax.grid(axis='y', alpha=0.3)

    ax2 = axes[1]
    bars = ax2.bar(edu_data['학력'], edu_data['평균점수'], color=edu_colors, alpha=0.85, width=0.5)
    ...
    ax2.set_ylabel('문해력 평균 점수'); ax2.set_ylim(150, 310); ax2.grid(axis='y', alpha=0.3)
    plt.tight_layout()
    st.pyplot(fig)
    plt.close()
...
def render_chart_box():
    ...
    for whisker in bp['whiskers']:
        whisker.set_color(GRAY); whisker.set_alpha(0.6)
    for cap in bp['caps']:
        cap.set_color(GRAY); cap.set_alpha(0.6)
...
# ════════════════════════════════════════════════
# 차트 10 — PIAAC 다중회귀 (신규 추가)
# ════════════════════════════════════════════════
def render_chart_regression():
    st.subheader("📊 차트 10 — PIAAC 개인단위 다중회귀분석: 연령·학력 통제 후 독서·ICT와 문해력")
    st.markdown("""
    ...
    **PIAAC 2023 한국 개인 단위 마이크로데이터(n≈6,000)**를 활용해
    ...
    > Rubin's rule로 계수·표준오차를 결합. 차트 8 편상관 분석의 다중회귀 확장 버전.
    """)

    # ── 사전 계산된 분석 결과값 ──────────────────────────────
    # M1: 연령만 / M2: +학력·성별 / M3: +독서·ICT
    R2 = {"M1\n연령만": 0.181, "M2\n+학력·성별": 0.251, "M3\n+독서·ICT": 0.283}
    ...
    # ── 왼쪽: R² 설명력 분해 막대 ───────────────────────────
    ax = axes[0]
    labels_r2 = list(R2.keys())
    vals_r2   = list(R2.values())
    bar_colors_r2 = [CORAL, BLUE, TEAL]

    bars = ax.bar(labels_r2, vals_r2, color=bar_colors_r2,
    ...
    # 증가량 화살표
    for x1, x2, y_pos, label in [
        (0, 1, 0.251, "+0.070\n(학력 추가)"),
        (1, 2, 0.283, "+0.032\n(독서·ICT 추가)"),
    ]:
        mid_x = (x1 + x2) / 2
        ax.annotate(
            "", xy=(x2 - 0.05, y_pos - 0.003), xytext=(x1 + 0.05, y_pos - 0.003),
            arrowprops=dict(arrowstyle="->", color=GRAY, lw=1.5)
        )
        ax.text(mid_x, y_pos + 0.008, label,
    ...
    for xi, color, txt in [(0, CORAL, "■ 연령"), (1, BLUE, "■ +학력"), (2, TEAL, "■ +독서·ICT")]:
        ax.text(xi, -0.035, txt, color=color, fontsize=9,
                ha="center", transform=ax.get_xaxis_transform())

    # ── 오른쪽: M3 계수 플롯 (성별 제외) ────────────────────
    ax2 = axes[1]
    plot_df = df_coef[df_coef["그룹"] != "성별"].copy().reset_index(drop=True)
    colors_coef = [GROUP_COLOR[g] for g in plot_df["그룹"]]
    y_pos = np.arange(len(plot_df))

    ax2.barh(y_pos, plot_df["계수"],
             xerr=[plot_df["계수"] - plot_df["CI_low"],
                   plot_df["CI_high"] - plot_df["계수"]],
             color=colors_coef, alpha=0.82, capsize=4, height=0.6)
    ax2.axvline(0, color="black", linewidth=0.9, linestyle="--")
    ax2.set_yticks(y_pos)
    ax2.set_yticklabels(
        [f"{row['변수']}  {row['기준']}" for _, row in plot_df.iterrows()],
        fontsize=8.5
    )
    ax2.invert_yaxis()
    ...
    display_df = df_coef[["그룹", "변수", "기준", "계수", "SE", "p", "유의"]].copy()
    ...
    st.caption("기준집단: 16-24세, 중졸이하, 남성 | 유의수준: *** p<.001  ** p<.01  * p<.05")
...
**연령 또는 세대와 관련된 다른 요인** — 코호트가 성장한
교육·문화 환경, 독서 습관 형성 시기 등 — 이
여전히 크게 남아 있음을 보여준다.
        """)
        st.markdown("#### ③ 독서활동 효과")
        st.success("""
독서활동지수는 연령·학력·성별·ICT를 모두 통제한 후에도
문해력과 **유의한 양의 관계**를 보였다 (β=+3.79, p<.001).

동일한 연령·학력 수준 내에서도 독서가 활발할수록
문해력이 높다는 것을 시사하며,
차트 8의 편상관(r=+0.234) 결과와 일관된다.
...
**⚠️ 해석 주의**
PIAAC의 ICT 이용지수(ICTHOMEC2)는 OTT·SNS 시청시간이
아니라 **이메일, 문서 작성, 정보 검색** 등
텍스트 기반 디지털 활동을 포함한 종합 지수다.

"디지털 미디어 이용이 문해력을 높인다"거나
"OTT·SNS가 문해력 저하로 이어지지 않는다"는
증거로 직접 사용하기에는 한계가 있다.
        """)

    # ── 대시보드 연결 ────────────────────────────────────────
    st.divider()
    st.markdown("### 🔗 기존 차트와의 연결")
    st.markdown("""
| 기존 차트 | 집계 수준 분석 내용 | 다중회귀 대응 |
|---|---|---|
| 차트 4 — 세대별 분포 | 연령대 간 평균 차이 확인 | **M1**: 연령만 투입 (R²=0.181) |
| 차트 6 — 디지털 네이티브 역설 | OTT-문해력 허위 상관, 연령 효과 | **M1→M2**: 학력 통제 전후 연령 계수 비교 |
| 차트 8 — 편상관 | 학력 통제 후 독서·ICT 효과 | **M3**: 동시 투입 후 순수 효과 분리 (R²=0.283) |
""")
...
분리하기 어렵다. 또한 회귀계수는 통제 후 조건부 연관성(conditional association)을
나타낼 뿐, 독서활동이나 ICT 이용이 문해력을 높인다는 인과 주장의 근거가 되지 않는다.
    """)

    # ── 최종 결론 ────────────────────────────────────────────
    st.divider()
    st.markdown("### 🎯 분석 프로젝트 최종 결론")
    st.markdown("""
    **제목: 한국 성인 문해력 격차의 구조적 원인 분석 —
    세대·학력·독서·디지털 미디어의 복합적 관련성**
    """)
    st.markdown("""
PIAAC 2023 한국 개인 단위 데이터(n≈6,000명)를 활용한 다중회귀분석 결과,
...
또한 독서활동지수는 통제변수를 포함한 이후에도 문해력과 유의한 양의 관계를
보여, 독서활동이 문해력과 독립적으로 관련될 가능성을 시사한다.
...
세 모델의 R² 변화(0.181 → 0.251 → 0.283)는 연령이 문해력 차이를 가장 많이 설명하고,
학력·성별이 추가 설명력을 더하며, 독서·ICT가 그 이후에도 독립적 관련성을 보임을
확인한다. 이 구조는 대시보드 차트 4·6·8의 흐름과 정확히 대응된다.
    """)
...
elif mode == "chart9":        render_chart9()
elif mode == "chart_box":     render_chart_box()
elif mode == "chart_regression": render_chart_regression()   # ← 신규 추가
