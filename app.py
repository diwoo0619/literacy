...
# ════════════════════════════════════════════════
def render_chart9():
    st.subheader("📊 차트 10 — PIAAC 개인단위: 학력 통제 후 독서·ICT ↔ 문해력 편상관")
    st.markdown("""
    기존 분석(집계 데이터 n=5)의 한계를 **PIAAC 개인 단위 데이터(n=6,145)**로 보완합니다.
    ...
    try:
        ...
        # TEXT 타입 컬럼 수치 변환
        for col in ['가정독서활동_지수','ICT_가정이용_지수','ICT_직장이용_지수','직장독서활동_지수']:
            df_p[col] = pd.to_numeric(df_p[col], errors='coerce')
    except Exception as e:
        st.error(f"PIAAC 데이터 로드 오류: {e}")
        return
    ...
    # 독서활동 vs 문해력
    v1 = df_p[['문해력_평균점수','가정독서활동_지수','학력_코드']].dropna()
    r1, p1 = pearsonr(v1['문해력_평균점수'], v1['가정독서활동_지수'])
    rl1 = v1['문해력_평균점수'] - v1.groupby('학력_코드')['문해력_평균점수'].transform('mean')
    rr1 = v1['가정독서활동_지수'] - v1.groupby('학력_코드')['가정독서활동_지수'].transform('mean')
    r1p, p1p = pearsonr(rl1, rr1)
    results['독서활동'] = {'단순': (r1, p1, len(v1)), '편상관': (r1p, p1p, len(v1))}

    # ICT 이용 vs 문해력
    v2 = df_p[['문해력_평균점수','ICT_가정이용_지수','학력_코드']].dropna()
    r2, p2 = pearsonr(v2['문해력_평균점수'], v2['ICT_가정이용_지수'])
    rl2 = v2['문해력_평균점수'] - v2.groupby('학력_코드')['문해력_평균점수'].transform('mean')
    rr2 = v2['ICT_가정이용_지수'] - v2.groupby('학력_코드')['ICT_가정이용_지수'].transform('mean')
    r2p, p2p = pearsonr(rl2, rr2)
    results['ICT이용'] = {'단순': (r2, p2, len(v2)), '편상관': (r2p, p2p, len(v2))}

    # ── 시각화 ─────────────────────────────────────────────
    fig, axes = plt.subplots(1, 2, figsize=(13, 5))

    labels  = ['독서활동↔문해력', 'ICT이용↔문해력']
    simple  = [results['독서활동']['단순'][0],  results['ICT이용']['단순'][0]]
    partial = [results['독서활동']['편상관'][0], results['ICT이용']['편상관'][0]]
    ns      = [results['독서활동']['단순'][2],   results['ICT이용']['단순'][2]]
    ...
    # p값 표시
    for xi, (rp, pp, _) in zip([0, 1], [results['독서활동']['편상관'], results['ICT이용']['편상관']]):
        sig = '***' if pp < 0.001 else ('**' if pp < 0.01 else '*')
        ax.text(xi + w/2, rp + 0.025, sig, ha='center', fontsize=12, color=TEAL)

    # 오른쪽: 학력별 문해력 분포 박스
    ax2 = axes[1]
    edu_order = ['중졸이하', '고졸', '대졸이상']
    edu_colors = [CORAL, AMBER, BLUE]
    bp_data = [df_p[df_p['학력']==e]['문해력_평균점수'].dropna().values for e in edu_order]
    bp = ax2.boxplot(bp_data, labels=edu_order, patch_artist=True, notch=False,
                     medianprops=dict(color='white', linewidth=2))
    for patch, color in zip(bp['boxes'], edu_colors):
        patch.set_facecolor(color)
        patch.set_alpha(0.75)

    for i, (data, label) in enumerate(zip(bp_data, edu_order)):
        ax2.text(i+1, np.mean(data)+3, f'평균\n{np.mean(data):.0f}점',
                 ha='center', fontsize=9, fontweight='bold')

    ax2.set_title('학력별 문해력 점수 분포\n(PIAAC 2023 한국)', fontsize=11, fontweight='bold')
    ...
    st.info(f"""
💡 **인사이트 (PIAAC 개인단위 분석)**
- 독서활동↔문해력: 학력 통제 전 r=+{simple[0]:.3f} → 학력 통제 후 r=+{partial[0]:.3f}. 학력 효과를 제거해도 독서와 문해력의 양의 상관이 유의하게 유지됩니다(p<0.0001).
