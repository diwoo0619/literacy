...
chart_options = {
    ...
    "차트 4 — 청년층 Gap 변화": "chart6",
    "차트 5 — OTT 이용 ↔ 문해력": "chart5",
    "차트 6 — 독서시간 ↔ 문해력": "chart4",
    "차트 7 — 독서 vs OTT 트레이드오프": "chart7",
...
- 코로나 이후에도 미달률이 회복되지 않고 있다는 점이 핵심입니다.
- 코로나가 직접적 원인임을 증명하는 것은 아니나, 언어 발달 환경과 상관관계가 있음을 시사합니다.
""")
...
- 그러나 이는 연령 자체의 효과(교육 수준, 인지 능력 등)의 영향이 있을 수 있어 독서만의 순수 효과로 해석하기 어렵습니다.
...
- OTT vs 문해력 상관(r = {r_ott:.3f})은 양의 방향으로, 미디어 이용과 문해력의 관계는 단순하지 않음을 시사합니다.
""")

def render_chart6():
    st.subheader("📊 차트 6 전체 평균 대비 연령대별 문해력 우위(Gap) 변화")
    st.markdown("""
    절대값이 아닌 **'전체 평균 대비 각 연령대의 격차(Gap)'** 를 봅니다.
    절대 점수가 상승해도 상대적 우위가 좁혀지고 있다면 문해력 하락의 신호입니다.
    """)
...
def render_chart7():
    st.subheader("📊 차트 7 : 연령대별 독서시간 vs OTT 이용시간 트레이드오프")
    st.markdown("""
