...
st.set_page_config(page_title="한국 문해력 저하 원인 분석", page_icon="📚", layout="wide")
...
if category == "분석1 : 세대·학력별 문해력 수준":
    chart_options = {
        ...
        "차트 3 — 세대별 문해력 분포":      "chart_box",   # ← 차트4에서 이동
        "차트 4 — 청년층 Gap 변화":  "chart6",      # ← 차트3에서 이동
    }
elif category == "분석2 : 연령대별 독서시간과 문해력 수준의 상관관계":
    chart_options = {
        "차트 7 — 독서시간 ↔ 문해력":             "chart4",
        "차트 8 — 독서 vs 미디어 트레이드오프": "chart7",
    }
...
# ════════════════════════════════════════════════
# 차트 4 — 전략A: Gap 변화 (구 차트3)
# ════════════════════════════════════════════════
def render_chart6():
    st.subheader("📊 차트 4 — 전체 평균 대비 연령대별 문해력 Gap 변화")
    st.markdown("""
...
def render_chart8():
    ...
    **"미디어·SNS를 많이 볼수록 문해력이 낮다"는 가설이 맞는가?**
    ...
    st.error("""
🔄 **역설처럼 보이는 이유**
미디어·SNS 이용률이 가장 높은 18~29세에서 문해력 수준4 비율도 97.3%로 가장 높습니다.
이것만 보면 "미디어를 많이 쓸수록 문해력이 높다"는 잘못된 결론에 도달할 수 있습니다.
    """)
    st.info("""
💡 **역설의 진짜 이유 — 연령 효과**
젊을수록 ① 교육 수준이 높고 ② 디지털 기기 이용도 많은 구조적 특성 때문입니다.
**연령 변수 하나가 문해력과 미디어 이용 모두를 동시에 설명**하는 허위 상관(spurious correlation)입니다.
    """)
    st.warning("""
⚠️ 개인 단위 데이터가 아닌 연령대 집계값 비교이므로, 이 결과로 개인 수준의 인과관계를 주장할 수 없습니다.
    """)
...
- 그러나 이는 연령 자체의 효과가 혼재할 수 있어 독서만의 순수 효과로 해석하기 어렵습니다.
...
# ════════════════════════════════════════════════
def render_chart7():
    st.subheader("📊 차트 8 — 연령대별 독서시간 vs 미디어 이용시간 트레이드오프")
    st.markdown("""
    **독서시간(감소)과 미디어 이용시간(증가)** 의 교차 패턴을 통해
    ...
    st.pyplot(fig); plt.close()
    show_sql(sql_ott)
    st.warning("⚠️ 독서시간(2023년)과 미디어 이용시간(2023년)은 서로 다른 조사에서 추출한 데이터입니다.")
    st.info(f"""
💡 **인사이트**
- 독서시간이 많은 연령대(18~29세: 36분)에서 미디어 이용시간도 가장 많아(221분/주), 단순한 '미디어 대체' 구도가 아님을 보여줍니다.
...
def render_chart_regression():
    ...
    R2 = {"M1\n연령만": 0.181, "M2\n+학력·성별": 0.251, "M3\n+독서·ICT": 0.283}
    coef_data = {
        "변수":  ["25-34세","35-44세","45-54세","55-65세","고졸","전문대이상","여성","독서활동","가정 ICT이용"],
        "그룹":  ["연령대","연령대","연령대","연령대","학력","학력","성별","독서·ICT","독서·ICT"],
    ...
    c2.metric("M2 R² (+학력·성별)", "0.251", delta="+0.070")
    c3.metric("M3 R² (+독서·ICT)", "0.283",  delta="+0.032")
    c4.metric("미설명 분산",         "71.7%")
...
코호트가 성장한 교육·문화 환경 등 **연령 또는 세대와 관련된 다른 요인**이
여전히 크게 남아 있음을 보여준다.
        """)
        st.markdown("#### ③ 독서활동 효과")
        st.success("""
...
단, 절댓값 기준으로 연령 계수(55-65세 −57.6)가 학력 계수(+33.1)보다 크므로,
...
⚠️ PIAAC의 ICT 이용지수는 OTT·SNS 시청시간이 아니라
**이메일, 문서 작성, 정보 검색** 등 텍스트 기반 디지털 활동을 포함한 종합 지수다.
        """)
